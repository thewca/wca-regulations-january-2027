...
- E3) The competitor may use the following objects during the attempt. Penalty for using unauthorized objects: disqualification of the attempt (DNF). Exception: if a non-permitted object is used, the current attempt and any completed attempts do not need be disqualified if the competitor did not gain an unfair advantage from the object, at the discretion of the WCA Delegate. However the competitor must not be permitted to perform any further attempts with the object.
